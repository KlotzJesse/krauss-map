...
type VersionTx = Parameters<typeof db.transaction>[0] extends (
  tx: infer T
) => Promise<unknown>
  ? T
  : never;
...
export async function createVersionAction(
  ...
  try {
    const result = await db.transaction((tx) =>
      createVersionWithTx(tx, areaId, data)
    );
...
export async function createVersionWithTx(
  ...
  await tx
    ...
    .where(
      and(eq(areaChanges.areaId, areaId), eq(areaChanges.versionAreaId, sql`NULL`))
    );
