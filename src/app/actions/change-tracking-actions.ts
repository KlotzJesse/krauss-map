...
/**
 * Pure DB primitive: record a change inside an existing transaction.
 * Callers are responsible for invalidating tags and triggering refresh.
 * Returns null when the area has no active version (change not recorded).
 */

export async function recordChangeWithTx(
  tx: Transaction,
  areaId: number,
  change: ChangeRecord
): Promise<ChangeKey | null> {
  ...
  // sequenceNumber is auto-generated by DB trigger
  const [newChange] = await tx
    .insert(areaChanges)
    .values({
      areaId,
      ...
      versionAreaId: areaId,
      versionNumber: area.currentVersionNumber,
  ...
  // Atomic upsert: append to undo stack and clear redo in one statement
  await tx
    .insert(areaUndoStacks)
    .values({
      areaId,
      undoStack: [changeKey],
      redoStack: [],
    ...
    .onConflictDoUpdate({
      target: areaUndoStacks.areaId,
      set: {
        undoStack: sql`${areaUndoStacks.undoStack} || ${JSON.stringify([changeKey])}::jsonb`,
        redoStack: sql`'[]'::jsonb`,
  ...
  await tx
    .update(areaVersions)
    .set({ changeCount: sql`${areaVersions.changeCount} + 1` })
    .where(
      and(
...
/**
 * Record a change with its own transaction. Use this when the mutation is
 * already committed (no larger outer transaction needed).
 *
 * Pass `invalidateHistory: false` for high-frequency operations where
 * immediate change-history freshness is not required.
 */
...
export async function recordChangeAction(
  ...
  try {
    const result = await db.transaction((tx) =>
      recordChangeWithTx(tx, areaId, change)
    );
