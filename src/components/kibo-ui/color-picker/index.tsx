...
export const ColorPicker = ({
  ...
  const [alpha, setAlpha] = useState(
    value ? selectedColor.alpha() * 100 : defaultColor.alpha() * 100 || 100
  );
...
export const ColorPickerEyeDropper = ({
  ...
  const handleEyeDropper = async () => {
    try {
      ...
      // EyeDropper not available or user cancelled
    }
  };
...
export const ColorPickerFormat = ({
  ...
  if (mode === "hex") {
    ...
    return (
      ...
      >
        <Input
          className="h-8 rounded-e-none bg-secondary px-2 text-xs shadow-none"
          readOnly
          type="text"
          value={hex}
        />
