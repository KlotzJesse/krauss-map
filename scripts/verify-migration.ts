...
async function verify() {
  ...
  for (const c of cols) {
    const col = c as any;
    console.log(`  ${col.table_name}.${col.column_name}: ${col.data_type} default=${col.column_default} nullable=${col.is_nullable}`);
  }
  ...
  for (const c of constraints) {
    const con = c as any;
    console.log(`  ${con.table_name}: ${con.conname} (${con.contype === 'u' ? 'UNIQUE' : con.contype === 'f' ? 'FK' : con.contype})`);
  }
