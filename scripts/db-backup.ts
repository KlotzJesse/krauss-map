...
async function backup() {
  ...
  const tables = [
    'postal_codes', 'states', 'areas', 'area_layers', 
    'area_layer_postal_codes', 'area_versions', 'area_changes',
    'area_undo_stacks'
  ];
  ...
  `);
  fs.writeFileSync(`${backupDir}/schema_info.json`, JSON.stringify(schemaInfo, null, 2));
  console.log(`\n✅ Schema info saved`);
