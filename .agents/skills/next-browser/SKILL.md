...
- `push` — shows what the client already has from prefetching. Requires
  ...
  so lock _after_ you've landed on the origin, not before.
...
fallback wrapping the whole content area renders _something_, but it's a
...
doing the prefetching — so `goto` the origin _unlocked_ and let it fully
